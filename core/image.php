    ...
    class c_image{
        ...
        public function resize($infile,$outfile,$width=0,$height=0,$bestfit=true,$corner=0,$quality=100,$sharp=true){
            ...
            if(in_array($type,array('gif','jpeg','jpg','png','bmp'))){
                ...
                if(!$im->scaleImage($width,$height,$bestfit)) return 'c_image_rsz_error_rsz_scale';
                if($sharp){
                    ...
                    if($sharp){
                        if(!$im->sharpenImage(1,4)) return 'c_image_rsz_error_sharp';
                    }
                }
                if($corner){
                    if(!$im->roundCorners($corner,$corner)) return 'c_image_rsz_error_corner';
                }
                if(!$im->writeImage($outfile)) return 'c_image_rsz_error_write';
            }
        ...
        public function resizeCrop($infile,$outfile,$width=0,$height=0,$corner=0,$quality=100){
            ...
            if(in_array($type,array('gif','jpeg','jpg','png','bmp'))){
                $im->setImageCompressionQuality($quality);
                if(!$im->cropThumbnailImage($width,$height)) return 'c_image_rsz_error_crop';
                if($corner){
                    if(!$im->roundCorners($corner,$corner)) return 'c_image_rsz_error_corner';
                }
                if(!$im->writeImage($outfile)) return 'c_image_rsz_error_write';
        ...
        public function thumbnail($infile,$outfile,$width,$height,$padding=0,$color='white'){
            ...
            $im=new Imagick();
            if(!$im->readImage($outfile)) return 'c_image_tn_error_read';
            ...
            if(!$canvas->compositeImage($im,imagick::COMPOSITE_OVER,$x,$y)) return 'c_image_tn_error_composite';
            if(!$canvas->writeImage($outfile)) return 'c_image_tn_error_write';
